<?php
	...
	function rss_storage($url) {
		...
		include_once('../../private/libraries/third-party/simplepie-1.8.0/SimplePie.compiled.php');
		$feed = new SimplePie();
		...
		foreach ($feed->get_items() as $item) {
			if ($item->get_title() && $item->get_date('Y-m-d H:i:s') && $item->get_description() && $item->get_permalink()) {
				...
				if ($item->get_enclosure()->get_link()) {
					include_once('../../private/utils/convertToWebp.php'); // Se añade la función de conversión de imágenes.
					$source = $item->get_enclosure()->get_link();
					$image = convertToWebp($source, crc32($title), 80, 700, 350);
				} else {
					$image = '../public/assets/Img/dummy_700x350.webp';
